  ...
  const tiers = [
    {
      ...
      color: "indigo",
      features: ["Personalized AI Story", "Pixar-style Hero Avatar", "Full HD PDF Download", "Access to Magic Mike Chat"]
    },
    {
      ...
      color: "pink",
      popular: true,
      features: ["Everything in Digital", "Premium Hardcover Book", "High-Quality Illustrations", "Global Shipping", "Personalized Dedication"]
    },
    {
      ...
      color: "yellow",
      features: ["Everything in Hardcover", "AI Voice Narration", "5 Bonus Adventure Pages", "Gold Foil Cover Lettering", "Lifetime PDF Updates"]
    }
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in zoom-in duration-500">
      <div className="text-center space-y-4">
        ...
        <p className="text-xl text-slate-600">Choose how you'd like to receive your adventure.</p>
      </div>
        ...
        {tiers.map((tier, idx) => (
          ...
          >
            ...
            <div className={`w-12 h-12 rounded-2xl mb-6 flex items-center justify-center ${
              ...
              tier.color === 'pink' ? 'bg-pink-100 text-pink-600' : 'bg-yellow-100 text-yellow-700'
            }`}>
              {tier.icon}
            </div>
            ...
            <div className="flex items-baseline gap-1 mb-4">
              <span className="text-4xl font-bubblegum text-slate-900">{tier.price}</span>
            </div>
            <p className="text-slate-500 text-sm mb-8">{tier.desc}</p>

            <div className="space-y-4 flex-1 mb-8">
              {tier.features.map((feat, fIdx) => (
                <div key={fIdx} className="flex gap-3 text-sm font-semibold text-slate-600">
      ...
      <p className="text-center text-slate-400 text-sm italic">
        * Prices include standard creation magic. Shipping calculated at checkout.
      </p>
    </div>
  );
};
